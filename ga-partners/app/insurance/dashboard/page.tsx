...
export default async function InsuranceDashboardPage() {
  const cookie = (await cookies()).get("insuranceToken")?.value;
  if (!cookie) {
    // not logged in
    return (
  ...
  if (!app) {
    return (
      <div className="p-8">
        ...
        <Link href="/apply/insurance" className="inline-block mt-4 px-4 py-2 bg-blue-600 text-white">Apply as Insurance</Link>
      </div>
    );
  }
